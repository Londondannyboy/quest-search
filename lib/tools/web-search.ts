...
// Serper search strategy (Quest Platform optimization)
class SerperSearchStrategy implements SearchStrategy {
  constructor(private serper: SerperClient) { }
  ...
  async search(
    ...
    const searchPromises = queries.map(async (query, index) => {
      ...
      try {
        ...
        const serperData = await this.serper.searchWithImages(query, {
          maxResults: currentMaxResults,
          includeNews: currentTopic === 'news',
        });

        const results = deduplicateByDomainAndUrl(serperData.results.map((obj) => ({
          url: obj.link,
          title: cleanTitle(obj.title || ''),
          content: obj.snippet,
          published_date: currentTopic === 'news' ? obj.date : undefined,
          author: undefined,
        })));

        const images = deduplicateByDomainAndUrl(serperData.images.map((obj) => ({
          url: obj.imageUrl,
          description: cleanTitle(obj.title || ''),
        })));
...
const createSearchStrategy = (
  provider: 'exa' | 'parallel' | 'tavily' | 'firecrawl' | 'serper',
  clients: {
    exa: Exa;
    parallel: Parallel;
  ...
  const strategies = {
    ...
    serper: () => new SerperSearchStrategy(clients.serper),
  };
...
export function webSearchTool(
  ...
  searchProvider: 'exa' | 'parallel' | 'tavily' | 'firecrawl' | 'serper' = 'serper',
) {
  return tool({
    ...
    execute: async ({
      ...
      const clients = {
        ...
        serper: new SerperClient(serverEnv.SERPER_API_KEY),
      };
